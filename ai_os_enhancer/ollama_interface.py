...
def analyze_system_item(item_content, item_path, item_type, model_name=None):
    ...
    prompt = (
        f"{prompt_intro}Your response MUST be a single, valid JSON object. Ensure all strings are properly escaped. The JSON object should have these top-level keys: "
...
def conceive_enhancement_strategy(system_snapshot, analysis_results_list, model_name=None):
    ...
    prompt = (
        ...
        "System Snapshot:\n"
        f"{_format_data_as_text(system_snapshot)}\n\n"
        "Analysis of System Items:\n"
        f"{_format_data_as_text(analysis_results_list)}\n\n"
...
if __name__ == '__main__':
    ...
    script_analysis = analyze_system_item(mock_script_content, "/tmp/mock_script.sh", "script")
    if script_analysis is not None:
        logger.info(f"Mock script analysis (Test 3): {json.dumps(script_analysis, indent=2)}")
    else:
        logger.error("Failed to get script analysis for Test 3.")
    
    # Test 4: Analyze system item (config)
    logger.info("Test 4: analyze_system_item (config)...")
    mock_config_content = "# Sample config\nTIMEOUT=30\nUSER=admin\n# Check this old_setting\nOLD_SETTING=true"
    config_analysis = analyze_system_item(mock_config_content, "/etc/mock_config.conf", "config")
    if config_analysis is not None:
        logger.info(f"Mock config analysis (Test 4): {json.dumps(config_analysis, indent=2)}")
    else:
    ...
    if script_analysis and isinstance(script_analysis.get("enhancement_ideas"), list):
         mock_analysis_results.append({
             "item_path": "/tmp/mock_script.sh", "item_type": "script", 
             "analysis_summary": script_analysis.get("analysis_summary"),
             "potential_issues": script_analysis.get("potential_issues"),
             "enhancement_ideas": script_analysis.get("enhancement_ideas")
        })
    if config_analysis and isinstance(config_analysis.get("enhancement_ideas"), list):
         mock_analysis_results.append({
             "item_path": "/etc/mock_config.conf", "item_type": "config", 
             "analysis_summary": config_analysis.get("analysis_summary"),
             "potential_issues": config_analysis.get("potential_issues"),
             "enhancement_ideas": config_analysis.get("enhancement_ideas")
        })
    
    if not mock_analysis_results: # Fallback if previous analyses failed
        mock_analysis_results.append({"item_path": "/etc/some.conf", "item_type": "config", "analysis_summary": "Outdated setting", "potential_issues": ["Security risk"], "enhancement_ideas": [{"idea_description": "Update setting X", "justification": "Improves security", "suggested_change_type": "modify_config_line", "target_pattern": "X=old", "new_code_snippet": "X=new"}]})

    strategy = conceive_enhancement_strategy(mock_snapshot, mock_analysis_results)
